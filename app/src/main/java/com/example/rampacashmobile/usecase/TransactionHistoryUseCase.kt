...
class TransactionHistoryUseCase @Inject constructor() {
    ...
    suspend fun getTransactionHistory(walletAddress: String): List<Transaction> =
        withContext(Dispatchers.IO) {
            try {
                ...
                val problematicTxSignature =
                    "5GoXQqV3T59LAh3FaCd7JSh7LPoyoy3BiDMkeWR3AVwaQsPJjvcsqFvcr5WCNn5EjvNXxbGmvQdnYY2u3ySHNbFh"
                if (signatures.contains(problematicTxSignature)) {
                    Log.d(TAG, "🔍 Found problematic transaction in signatures!")
                } else {
                    Log.d(
                        TAG,
                        "⚠️ Problematic transaction NOT found in signatures for $walletAddress"
                    )
                    // Try to fetch this specific transaction directly
                    try {
                        val specificTx =
                            getTransactionDetails(problematicTxSignature, walletAddress)
                        if (specificTx != null) {
                            Log.d(
                                TAG,
                                "🔍 Successfully parsed problematic transaction directly: ${specificTx.amount} ${specificTx.tokenSymbol} ${specificTx.transactionType}"
                            )
                        } else {
                    ...
                    try {
                        ...
                        if (transaction != null) {
                            ...
                            if (signature.startsWith("5GoXQqV3T59LAh3FaCd7JSh7LPoyoy3BiDMkeWR3AVwaQsPJjvcsqFvcr5WCNn5EjvNXxbGmvQdnYY2u3ySHNbFh")) {
                                Log.d(
                                    TAG,
                                    "🔍 Target transaction parsed: ${transaction.amount} ${transaction.tokenSymbol} ${transaction.transactionType}"
                                )
                            }
                        } else {
                            if (signature.startsWith("5GoXQqV3T59LAh3FaCd7JSh7LPoyoy3BiDMkeWR3AVwaQsPJjvcsqFvcr5WCNn5EjvNXxbGmvQdnYY2u3ySHNbFh")) {
                        ...
                        Log.w(
                            TAG,
                            "⚠️ Failed to get details for signature $signature: ${e.message}"
                        )
                        if (signature.startsWith("5GoXQqV3T59LAh3FaCd7JSh7LPoyoy3BiDMkeWR3AVwaQsPJjvcsqFvcr5WCNn5EjvNXxbGmvQdnYY2u3ySHNbFh")) {
    ...
    private suspend fun getConfirmedSignatures(address: String): List<String> =
        withContext(Dispatchers.IO) {
            try {
    ...
    private suspend fun getTransactionDetails(
        ...
        try {
            ...
            if (jsonResponse.has("error")) {
                Log.w(
                    TAG,
                    "RPC Error for transaction $signature: ${jsonResponse.getJSONObject("error")}"
                )
            ...
            meta?.let { metadata ->
                if (metadata.has("postTokenBalances") && !metadata.isNull("postTokenBalances") &&
                        ...
                        if (postBalance.has("mint") && postBalance.has("owner")) {
                            ...
                            if (owner == walletAddress) {
                                ...
                                currentTokenType?.let { tokenType ->
                                    ...
                                    try {
                                        val uiTokenAmount =
                                            postBalance.getJSONObject("uiTokenAmount")
                                        val postAmount = if (uiTokenAmount.isNull("uiAmount")) {
                                            ...
                                            if (uiTokenAmount.has("amount")) {
                                                val rawAmount = uiTokenAmount.getString("amount")
                                                    .toDoubleOrNull() ?: 0.0
                                        ...
                                        for (j in 0 until preTokenBalances.length()) {
                                            ...
                                            if (preBalance.has("owner") && preBalance.getString("owner") == owner &&
                                                ...
                                                val preUiTokenAmount =
                                                    preBalance.getJSONObject("uiTokenAmount")
                                                val preAmount =
                                                    if (preUiTokenAmount.isNull("uiAmount")) {
                                                        Log.d(
                                                            TAG,
                                                            "⚠️ Pre uiAmount is null, trying amount..."
                                                        )
                                                        if (preUiTokenAmount.has("amount")) {
                                                            val rawAmount =
                                                                preUiTokenAmount.getString("amount")
                                                                    .toDoubleOrNull() ?: 0.0
                                                            val decimals = preUiTokenAmount.optInt(
                                                                "decimals",
                                                                6
                                                            )
                                                            rawAmount / Math.pow(
                                                ...
                                                if (amountChange > 0.0001) { // Significant change
                                                    val direction =
                                                        if (postAmount > preAmount) "RECEIVED" else "SENT"

                                                    Log.d(
                                                        TAG,
                                                        "📊 $tokenType balance change: $preAmount -> $postAmount ($direction) Amount: $amountChange"
                                                    )
                                                    ...
                                                    if (walletAddress == "DLCvDmn2t294CseF87Q3YscSNritr7szsYraMp16oEEG") {
                                                        Log.d(
                                                            TAG,
                                                            "🔍 $tokenType $direction: $preAmount -> $postAmount, change: $amountChange, mint: $mint"
                                                        )
                                                    }
                                                    ...
                                                    if (amountChange > totalAmountChange) {
                                                        ...
                                                        if (walletAddress == "DLCvDmn2t294CseF87Q3YscSNritr7szsYraMp16oEEG") {
                                                            Log.d(
                                                                TAG,
                                                                "🔍 Setting as PRIMARY: $tokenType $direction $amountChange"
                                                            )
                                                        }
                                                    }
                                                }
                                        ...
                                        Log.w(
                                            TAG,
                                            "⚠️ Failed to parse balance amounts: ${e.message}"
                                        )
                                    }
                                }
                            }
                    ...
                    if (primaryTokenType != null && primaryDirection != null && totalAmountChange > 0) {
                        ...
                        Log.d(
                            TAG,
                            "✅ Final transaction: $primaryTokenType $primaryDirection $totalAmountChange"
                        )

                        if (walletAddress == "DLCvDmn2t294CseF87Q3YscSNritr7szsYraMp16oEEG") {
                            Log.d(
                                TAG,
                                "🔍 FINAL RESULT: $primaryTokenType $primaryDirection $totalAmountChange"
                            )
                        }
                    }
                } else {
                    ...
                    for (i in 0 until metaAccountKeys.length()) {
                        if (metaAccountKeys.getString(i) == walletAddress) {
                            if (i < preBalances.length() && i < postBalances.length()) {
                                val preBalance =
                                    preBalances.getLong(i) / 1_000_000_000.0 // Convert lamports to SOL
                                ...
                                if (Math.abs(preBalance - postBalance) > 0.0001) { // Significant SOL change
                                    ...
                                    Log.d(
                                        TAG,
                                        "💰 Detected SOL transfer: $preBalance -> $postBalance (Amount: $actualTransactionAmount)"
                                    )
                                    break
                                }
                            }
            ...
            if (detectedTokenFromMeta != null) {
                Log.d(
                    TAG,
                    "📊 Transaction ${signature.take(8)} - Token: $detectedTokenFromMeta, Amount: $actualTransactionAmount, Direction: $balanceChangeDirection"
                )
            }
                    ...
                    if (metadata.has("preTokenBalances") && metadata.has("postTokenBalances")) {
                        ...
                        for (i in 0 until postTokenBalances.length()) {
                            ...
                            if (postBalance.has("owner") && postBalance.getString("owner") == walletAddress) {
                                val mint =
                                    if (postBalance.has("mint")) postBalance.getString("mint") else "unknown"
                                val postAmount = if (postBalance.has("uiTokenAmount"))
                                    postBalance.getJSONObject("uiTokenAmount")
                                        .getDouble("uiAmount") else 0.0
                                ...
                                for (j in 0 until preTokenBalances.length()) {
                                    ...
                                    if (preBalance.has("owner") && preBalance.getString("owner") == walletAddress &&
                                        ...
                                        preAmount = preBalance.getJSONObject("uiTokenAmount")
                                            .getDouble("uiAmount")
                                        break
                                    }
                                }
                                ...
                                Log.d(
                                    TAG,
                                    "🔍 - Token $mint: $preAmount -> $postAmount (change: $change)"
                                )
                            }
                        }
                    }
    ...
    private fun parseTransaction(
        ...
        try {
            ...
            val blockTime =
                if (transactionResult.has("blockTime") && !transactionResult.isNull("blockTime")) {
            ...
            if (walletIndex == -1) {
                if (detectedTokenFromMeta != null && actualTransactionAmount != null && actualTransactionAmount > 0) {
                    Log.d(
                        TAG,
                        "🔍 Wallet not in accounts but metadata shows involvement - continuing with metadata-only parsing"
                    )
                    // We'll use metadata-only parsing for this transaction
                } else {
                    Log.w(
                        TAG,
                        "Wallet address not found in transaction accounts and no metadata - skipping"
                    )
                    return null
                }
            }
            ...
            for (i in 0 until instructions.length()) {
                ...
                val instructionData =
                    if (instruction.has("data")) instruction.getString("data") else ""
        ...
        try {
            ...
            if (accounts.length() >= 2) {
                ...
                val amount = actualTransactionAmount?.also {
                    ...
                    Log.d(
                        TAG,
                        "⚠️ Using ${if (parsedAmount > 0.0) "parsed" else "fallback"} SOL amount: $fallbackAmount SOL"
                    )
                    fallbackAmount
                }
    ...
    private fun parseSplTokenTransfer(
        ...
        try {
            ...
            if (accounts.length() >= 2) {
                ...
                Log.d(
                    TAG,
                    "🔍 Extracted addresses - From: ${fromAddress.take(8)}..., To: ${toAddress.take(8)}..."
                )
            }
            ...
            if (!balanceChangeDirection.isNullOrEmpty()) {
                ...
                transactionType =
                    TransactionType.RECEIVED // Default to received when we have amount but unclear direction
                Log.d(
                    TAG,
                    "🔍 Have amount ($actualTransactionAmount) but no direction, defaulting to RECEIVED"
                )
            } else {
                ...
                if (accounts.length() >= 2) {
                    ...
                    if (walletFoundIndex != -1) {
                        ...
                        if (walletInvolvedAsIndex == 0) {
                            ...
                            Log.d(
                                TAG,
                                "🔍 Wallet found as source authority at position $walletInvolvedAsIndex -> SENT"
                            )
                        } else {
                            ...
                            Log.d(
                                TAG,
                                "🔍 Wallet found at position $walletInvolvedAsIndex -> RECEIVED (conservative)"
                            )
                        }
                    } else {
            ...
            Log.d(
                TAG,
                "🔍 Final addresses - From: ${fromAddress.take(8)}..., To: ${toAddress.take(8)}..."
            )
            ...
            val amount = actualTransactionAmount?.also {
                ...
                Log.d(
                    TAG,
                    "⚠️ Using ${if (parsedAmount > 0.0) "parsed" else "fallback"} amount: $fallbackAmount $tokenSymbol"
                )
                if (walletAddress == "DLCvDmn2t294CseF87Q3YscSNritr7szsYraMp16oEEG") {
                    Log.d(
                        TAG,
                        "🔍 DEBUG: No metadata amount, using ${if (parsedAmount > 0.0) "parsed" else "fallback"} $fallbackAmount for $tokenSymbol"
                    )
                    Log.d(
                        TAG,
                        "🔍 DEBUG: detectedTokenFromMeta=$detectedTokenFromMeta, balanceChangeDirection=$balanceChangeDirection"
                    )
                }
                fallbackAmount
            }
    ...
    private fun parseAmountFromInstructionData(
        data: String,
                ...
                if (decodedData[0] == 3.toByte()) {
                    ...
                    for (i in 1..8) {
                        if (i < decodedData.size) {
                            amount =
                                amount or ((decodedData[i].toLong() and 0xFF) shl ((i - 1) * 8))
                        }
                    }
