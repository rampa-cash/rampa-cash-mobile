...
object TokenAccountBalanceUseCase {
    ...
    suspend operator fun invoke(rpcUri: Uri, tokenAccount: SolanaPublicKey): Long =
        withContext(Dispatchers.IO) {
            ...
            response.error?.let { error ->
                val errorMessage = "Could not fetch token balance for account [${tokenAccount.base58()}]: ${error.code}, ${error.message}"
            ...
            Log.d(TAG, "getTokenBalance tokenAccount=${tokenAccount.base58()}, amount=$tokenAmount, decimals=$decimals")
    ...
    open class TokenBalanceException(message: String? = null, cause: Throwable? = null) : RuntimeException(message, cause)
    class TokenAccountNotFoundException(message: String? = null, cause: Throwable? = null) : TokenBalanceException(message, cause)
    class InvalidTokenAccountException(message: String? = null, cause: Throwable? = null) : TokenBalanceException(message, cause)
} 